...
### Q: 需要配 AI 的 API key 吗？

A: **不用**。Skill 默认会自动降级到本地关键词匹配模式，足够处理 80% 的召回风险。
- 零配置：直接 `python run.py` 就能跑出报告
- 想要更精准的判断：配 `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GOOGLE_API_KEY` 任意一个环境变量
- 不配也能用：会看到日志里"AI 调用失败，降级到 mock 模式"，这是正常的

实测：用 13 个示例 SKU + 没配 API key，抓到 10 条 CPSC 公告，识别出 8 个高风险 + 2 个中风险，主题命中 LED/电池/儿童。
